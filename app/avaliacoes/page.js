    ...
    <AppLayout userName={userName}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
        <div>
          ...
          <VideoTutorialLink slug="avaliacoes" />
        </div>
                  ...
                  <tr key={e.id} style={{ borderTop: '1px solid var(--border-card)', background: selected.has(e.id) ? 'var(--bg-content)' : 'none' }}>
                    ...
                    <td style={{ padding: '13px 16px', textAlign: 'right' }}>
                      <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                        <button onClick={() => window.open(`/relatorio-individual?id=${e.id}`, '_blank')} style={{ padding: '5px 12px', border: '1px solid var(--border)', borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: 'pointer', background: 'var(--bg-content)', color: '#0081f0', fontFamily: 'inherit' }}>Ver</button>
                        <button onClick={() => window.open(`/relatorio-individual?id=${e.id}&print=1`, '_blank')} style={{ padding: '5px 12px', border: '1px solid var(--border)', borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: 'pointer', background: 'var(--bg-content)', color: 'var(--text-main)', fontFamily: 'inherit' }}>Baixar PDF</button>
                        <button onClick={() => { setDetail(e); setDetailDraft(JSON.parse(JSON.stringify(e))); }} style={{ padding: '5px 12px', border: '1px solid var(--border)', borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: 'pointer', background: 'var(--bg-content)', color: 'var(--text-main)', fontFamily: 'inherit' }}>Editar</button>
                        <button onClick={() => del(e.id)} style={{ padding: '5px 9px', border: '1px solid #EF444433', borderRadius: 7, fontSize: 12, cursor: 'pointer', background: 'transparent', color: '#EF4444', display: 'flex', alignItems: 'center', fontFamily: 'inherit' }}>
          ...
          <div onClick={() => { setDetail(null); setDetailDraft(null); }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
            <div onClick={ev => ev.stopPropagation()} style={{ background: 'var(--bg-card)', borderRadius: 14, padding: 32, maxWidth: 680, width: '100%', maxHeight: '85vh', overflowY: 'auto', border: '1px solid var(--border-card)' }}>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <button onClick={() => window.open(`/relatorio-individual?id=${d.id}&print=1`, '_blank')} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 20px', background: 'linear-gradient(135deg, #0081f0, #0033ad)', color: 'white', border: 'none', borderRadius: 9, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
