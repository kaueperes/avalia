...
// ── Semestre (1º: jan-jun, 2º: jul-dez) — mesmo corte simples do /inicio ────
...
export default function OrgDashboardPage() {
  ...
  const [orgInfo, setOrgInfo] = useState(null); // só admin
  ...
  useEffect(() => {
    ...
    try {
      ...
      admin = u.org_role === 'admin';
      setIsAdmin(admin);
    } catch { router.push('/inicio'); return; }

    async function load() {
      const quotaRes = await fetch('/api/org/quota', { headers: { Authorization: `Bearer ${token()}` } });
      const quotaData = quotaRes.ok ? await quotaRes.json() : null;
      const coordinator = !!quotaData?.isCoordinator;
      setIsCoordinator(coordinator);

      if (!admin && !coordinator) { router.push('/inicio'); return; }

      const requests = [
        fetch('/api/org/evaluations', { headers: { Authorization: `Bearer ${token()}` } }),
      ];
      if (admin) requests.push(fetch('/api/org/dashboard', { headers: { Authorization: `Bearer ${token()}` } }));

      const results = await Promise.all(requests);
      const evalData = results[0].status === 403 ? [] : await results[0].json();
      setEvaluations(Array.isArray(evalData) ? evalData.map(e => ({
      ...
      if (admin && results[1]) {
        const orgData = results[1].status === 403 ? null : await results[1].json();
        setOrgInfo(orgData);
      }
      setLoading(false);
    }
  ...
  // Avaliações do período selecionado — mesmo escopo do /inicio individual
  ...
  const allTeachers = useMemo(
    () => [...new Set(periodEvals.map(e => e.teacherName).filter(Boolean))],
    [periodEvals]
  );
  ...
  const kpis = [
    { label: 'Total de avaliações', value: loading ? '—' : evaluations.length, sub: isAdmin ? 'de toda a instituição' : 'da sua equipe', color: 'var(--text-main)' },
    { label: 'Média geral', value: loading ? '—' : (avg ?? '—'), sub: 'de todas as avaliações', color: avg ? scoreColor(parseFloat(avg)) : 'var(--text-main)' },
    { label: 'Taxa de aprovação', value: loading ? '—' : `${aprovPct}%`, sub: `${aprovados} de ${evaluations.length} alunos`, color: '#16a34a' },
    { label: 'Professores', value: loading ? '—' : allTeachers.length, sub: isAdmin ? 'ativos na instituição' : 'na sua equipe', color: '#810cfa' },
    { label: 'Em atenção', value: loading ? '—' : emAtencao.length, sub: 'alunos com nota < 6', color: !loading && emAtencao.length > 0 ? '#ef4444' : '#16a34a', alert: !loading && emAtencao.length > 0 },
  ];
    ...
    <AppLayout userName={userName}>
      <div style={{ marginBottom: 28 }}>
        ...
        <h1 style={{ fontSize: 28, fontWeight: 800, color: 'var(--text-main)', letterSpacing: '-0.5px' }}>
          {isAdmin ? (orgInfo?.org?.name || 'Minha Instituição') : 'Painel da Equipe'}
        </h1>
        <p style={{ fontSize: 15, color: 'var(--text-muted)', marginTop: 4 }}>
          {isAdmin ? 'Visão geral de todas as correções da instituição.' : 'Visão geral das correções dos professores da sua equipe.'}
        </p>
      </div>
      ...
      {!loading && (
        <>
          ...
          {evaluations.length === 0 ? (
            <div style={{ background: 'var(--bg-card)', borderRadius: 14, border: '1px solid var(--border-card)', padding: '48px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 14, color: 'var(--text-muted)' }}>
                {isAdmin ? 'Assim que os professores da instituição começarem a corrigir, os dados aparecem aqui.' : 'Assim que os professores da sua equipe começarem a corrigir, os dados aparecem aqui.'}
              </p>
            </div>
          ) : (
            <>
              <div className="grid-collapse" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 14, marginBottom: 20 }}>
                {kpis.map((kpi, i) => (
                  <div key={i} style={{ background: kpi.alert ? '#fff5f5' : 'var(--bg-card)', borderRadius: 14, padding: '18px 20px', border: `1px solid ${kpi.alert ? '#fecaca' : 'var(--border-card)'}`, display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 32, fontWeight: 900, color: kpi.color, lineHeight: 1, letterSpacing: '-1.5px' }}>{kpi.value}</span>
              ...
              <div className="grid-collapse" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                ...
                <div style={{ background: 'var(--bg-card)', borderRadius: 14, border: '1px solid var(--border-card)', padding: '22px 24px' }}>
                  <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-main)', marginBottom: 4 }}>Distribuição de notas</h3>
                  <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 20 }}>Faixas de desempenho</p>
                      ...
                      {filteredAtencao.slice(0, 5).map((e, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: '#fff5f5', borderRadius: 10, border: '1px solid #fecaca', borderLeft: '3px solid #ef4444' }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>{e.teacherName ? `${e.teacherName} · ` : ''}{e.turma ? `${e.turma} · ` : ''}{e.exerciseName || e.type}</p>
                          </div>
                  ...
                  <div style={{ background: 'var(--bg-card)', borderRadius: 14, border: '1px solid var(--border-card)', padding: '22px 24px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {turmaSummaries.map((t, i) => {
                        ...
                        return (
                          <div key={i} onClick={() => setSelectedTurma(isSelected ? '' : t.name)}
              ...
              {recent.length > 0 && (
                <div style={{ background: 'var(--bg-card)', borderRadius: 14, border: '1px solid var(--border-card)', padding: '22px 24px', marginBottom: 24 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                    <h3 style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-main)' }}>Avaliações recentes</h3>
                    <button onClick={() => router.push('/org/avaliacoes')} style={{ fontSize: 13, fontWeight: 600, color: '#0081f0', background: 'none', border: 'none', cursor: 'pointer' }}>Ver todas →</button>
                  </div>
                  ...
                  {recent.map((e, i) => {
                    ...
                    return (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 0', borderBottom: i < recent.length - 1 ? '1px solid var(--border)' : 'none' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-main)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{e.studentName}</p>
                          <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>{e.teacherName ? `${e.teacherName} · ` : ''}{e.exerciseName || e.type}{e.turma ? ` · ${e.turma}` : ''}</p>
                        </div>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            {isAdmin && (
              <button onClick={() => router.push('/org/professores')} style={{ padding: '11px 24px', background: 'linear-gradient(135deg, #0081f0, #0033ad)', color: 'white', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                Gerenciar Professores
              </button>
            )}
            <button onClick={() => router.push('/org/avaliacoes')} style={{ padding: '11px 24px', background: 'var(--bg-card)', color: 'var(--text-main)', border: '1px solid var(--border)', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
              Ver Avaliações
            </button>
          </div>
        </>
