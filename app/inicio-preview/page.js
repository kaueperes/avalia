...
// ── Mock data ─────────────────────────────────────────────────────────────────
const EVALUATIONS = [
  { studentName: 'Ana Souza',       turma: '3º A', exerciseName: 'Redação — Meio Ambiente', score: 8.5, createdAt: '2026-03-25T10:00:00Z' },
  { studentName: 'Bruno Lima',      turma: '3º A', exerciseName: 'Redação — Meio Ambiente', score: 5.5, createdAt: '2026-03-25T11:00:00Z' },
  { studentName: 'Carla Mendes',    turma: '3º B', exerciseName: 'Interpretação Textual',   score: 9.2, createdAt: '2026-03-24T09:00:00Z' },
  { studentName: 'Diego Rocha',     turma: '3º B', exerciseName: 'Interpretação Textual',   score: 5.0, createdAt: '2026-03-24T14:00:00Z' },
  { studentName: 'Elena Castro',    turma: '2º A', exerciseName: 'Gramática Avançada',       score: 7.8, createdAt: '2026-03-23T10:00:00Z' },
  { studentName: 'Felipe Neto',     turma: '2º A', exerciseName: 'Gramática Avançada',       score: 8.5, createdAt: '2026-03-23T11:00:00Z' },
  { studentName: 'Gabriela Santos', turma: '3º A', exerciseName: 'Redação — Tecnologia',     score: 7.2, createdAt: '2026-03-22T10:00:00Z' },
  { studentName: 'Henrique Pires',  turma: '3º B', exerciseName: 'Interpretação Textual',   score: 4.5, createdAt: '2026-03-22T14:00:00Z' },
  { studentName: 'Isabela Faria',   turma: '2º A', exerciseName: 'Gramática Avançada',       score: 9.5, createdAt: '2026-03-21T10:00:00Z' },
  { studentName: 'João Pedro',      turma: '3º A', exerciseName: 'Redação — Meio Ambiente', score: 7.5, createdAt: '2026-03-20T10:00:00Z' },
  { studentName: 'Karina Dias',     turma: '2º B', exerciseName: 'Literatura Brasileira',   score: 9.0, createdAt: '2026-03-19T10:00:00Z' },
  { studentName: 'Lucas Martins',   turma: '2º B', exerciseName: 'Literatura Brasileira',   score: 6.5, createdAt: '2026-03-18T10:00:00Z' },
];

const TURMA_COLORS = {
  '3º A': { color: '#0081f0', bg: '#eff6ff' },
  '3º B': { color: '#d97706', bg: '#fffbeb' },
  '2º A': { color: '#16a34a', bg: '#f0fdf4' },
  '2º B': { color: '#7c3aed', bg: '#f5f3ff' },
};
...
function EvolutionChart({ data, color = '#0081f0' }) {
  ...
  const gridLines = [4, 6, 8, 10].map(v => {
...
// ── Page ──────────────────────────────────────────────────────────────────────
export default function InicioPreviewPage() {
  const [selectedTurma, setSelectedTurma] = useState('');

  const allTurmas = [...new Set(EVALUATIONS.map(e => e.turma))].sort();
  ...
  // KPI stats (always global)
  const scores = EVALUATIONS.map(e => e.score);
  const avg = (scores.reduce((a, b) => a + b, 0) / scores.length).toFixed(1);
  const aprovados = scores.filter(s => s >= 6).length;
  const aprovPct = Math.round((aprovados / scores.length) * 100);
  ...
  // Trend badge for evolution chart
  ...
  // Filtered atenção & alunos em atenção
  const filteredAtencao = filteredEvals.filter(e => e.score < 6).sort((a, b) => a.score - b.score);

  // Turma summaries
  const turmaSummaries = allTurmas.map(t => {
    const evals = EVALUATIONS.filter(e => e.turma === t);
    const tScores = evals.map(e => e.score);
    const tMedia = parseFloat((tScores.reduce((s, v) => s + v, 0) / tScores.length).toFixed(1));
    const tAprov = Math.round((tScores.filter(s => s >= 6).length / tScores.length) * 100);
    return { name: t, total: evals.length, media: tMedia, aprovPct: tAprov, scores: tScores, ...TURMA_COLORS[t] };
  });

  // Chart accent color based on selected turma
  const chartColor = selectedTurma ? (TURMA_COLORS[selectedTurma]?.color || '#0081f0') : '#0081f0';

  const kpis = [
    { label: 'Total de avaliações', value: EVALUATIONS.length, sub: 'todas as avaliações', trend: '+11 esta semana', color: 'var(--text-main)', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" stroke="#6b7280" strokeWidth="1.5" strokeLinecap="round"/></svg> },
    { label: 'Média geral', value: avg, sub: 'de todas as avaliações', color: scoreColor(parseFloat(avg)), icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" stroke="#6b7280" strokeWidth="1.5" strokeLinecap="round"/></svg> },
    { label: 'Taxa de aprovação', value: `${aprovPct}%`, sub: `${aprovados} de ${EVALUATIONS.length} alunos`, color: '#16a34a', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" stroke="#6b7280" strokeWidth="1.5" strokeLinecap="round"/></svg> },
    { label: 'Alunos avaliados', value: uniqueStudents, sub: 'alunos únicos', color: '#810cfa', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" stroke="#6b7280" strokeWidth="1.5" strokeLinecap="round"/></svg> },
    { label: 'Em atenção', value: emAtencao.length, sub: 'alunos com nota < 6', color: emAtencao.length > 0 ? '#ef4444' : '#16a34a', alert: emAtencao.length > 0, icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M12 9v4m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" stroke="#6b7280" strokeWidth="1.5" strokeLinecap="round"/></svg> },
  ];
    ...
    <AppLayout userName="Kauê">
        ...
        {kpis.map((kpi, i) => (
          <div key={i} style={{ background: kpi.alert ? '#fff5f5' : 'var(--bg-card)', borderRadius: 14, padding: '18px 20px', border: `1px solid ${kpi.alert ? '#fecaca' : 'var(--border-card)'}`, display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8 }}>
              ...
              {kpi.trend && (
                <span style={{ fontSize: 11, fontWeight: 700, color: '#16a34a', background: '#f0fdf4', padding: '2px 8px', borderRadius: 20, marginBottom: 3, border: '1px solid #bbf7d0' }}>
                  ↑ {kpi.trend}
                </span>
              )}
            </div>
        ...
        {selectedTurma && (
          <>
            <span style={{
              ...
              background: TURMA_COLORS[selectedTurma]?.bg,
              color: TURMA_COLORS[selectedTurma]?.color,
              border: `1px solid ${TURMA_COLORS[selectedTurma]?.color}33`,
            }}>
              {selectedTurma} · {filteredEvals.length} avaliações
            </span>
            <button
              onClick={() => setSelectedTurma('')}
              style={{ fontSize: 12, color: 'var(--text-muted)', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 6px', fontFamily: 'inherit' }}
            >
              × Limpar
            </button>
          </>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {trendDelta !== null && (
                <span style={{
                  ...
                  color: trendDelta > 0 ? '#16a34a' : trendDelta < 0 ? '#ef4444' : '#6b7280',
                  background: trendDelta > 0 ? '#f0fdf4' : trendDelta < 0 ? '#fef2f2' : '#f1f5f9',
                  border: `1px solid ${trendDelta > 0 ? '#bbf7d0' : trendDelta < 0 ? '#fecaca' : '#e2e8f0'}`,
                }}>
              ...
              {filteredAtencao.map((e, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: '#fff5f5', borderRadius: 10, border: '1px solid #fecaca', borderLeft: '3px solid #ef4444' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>{e.turma} · {e.exerciseName}</p>
                  </div>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    background: isSelected ? t.bg : 'var(--bg-content)',
                    borderRadius: 10,
                    border: `1px solid ${isSelected ? t.color + '44' : 'var(--border)'}`,
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                      ...
                      <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>{t.total} alunos</p>
                    </div>
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 14, border: '1px solid var(--border-card)', padding: '22px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
          ...
          <button style={{ fontSize: 13, fontWeight: 600, color: '#0081f0', background: 'none', border: 'none', cursor: 'pointer' }}>Ver todas →</button>
        </div>
        ...
        {recent.map((e, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 0', borderBottom: i < recent.length - 1 ? '1px solid var(--border)' : 'none' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-main)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{e.studentName}</p>
                <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>{e.exerciseName} · {e.turma}</p>
              </div>
      ...
      <div style={{ marginTop: 20, textAlign: 'center', fontSize: 12, color: 'var(--text-muted)' }}>
        Página de preview com dados fictícios · <a href="/inicio" style={{ color: '#0081f0' }}>← Voltar ao início atual</a>
      </div>
