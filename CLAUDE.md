...
- `PostHogTracker` (client component, montado em `app/layout.js` junto do Meta Pixel) identifica o usuário logado (`phIdentify`, via `localStorage.user`) no mount. Pageview/pageleave em SPA (App Router) **não são disparados manualmente** — `capture_pageview: 'history_change'` em `lib/posthog.js` faz o próprio SDK escutar a History API e parear os dois eventos corretamente (jeito recomendado pela doc oficial pra SPA; disparar pageview manual via `capture()` deixa o pageleave automático descasado, foi o que causava o aviso "$pageleave" na página Health do PostHog)
...
- **Campanha paga no Meta Ads (Facebook/Instagram) — criada e no ar** (set/2026). Infra: Business Manager, Pixel instalado e testado, domínio verificado, Página do Facebook e Instagram conectados. Destino do anúncio pode ser a home (`/`) ou a nova landing `/experimente` (teste sem cadastro) — ver [[landing-experimente]]. Atenção a emails de phishing se passando por "Meta Advertising Compliance" (remetente falso, ameaças vagas de violação de política, link pra página de login falsa) — contas de anúncio novas viram alvo; conferir status só em `business.facebook.com`, nunca por link de email.
